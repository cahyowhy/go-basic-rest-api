...
func ProcessJSON(w http.ResponseWriter, status int, response []byte, statuscode string, count string) {
	responseJson := response
	if len(statuscode) != 0 {
		responseJson, _ = handleJson(response, statuscode, count)
	}
...
func handleJson(payload []byte, status string, count string) ([]byte, error) {
	...
	if len(count) > 0 {
		clonedJson, err = sjson.SetRawBytes(clonedJson, "count", []byte(fmt.Sprintf(`"%s"`, count)))
	}
